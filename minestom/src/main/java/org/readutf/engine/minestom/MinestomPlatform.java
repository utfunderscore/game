...
public class MinestomPlatform implements GamePlatform {
    ...
    @Override
    public @NotNull CompletableFuture<Void> teleport(@NotNull UUID playerId, @NotNull Position position) {
        Player player = getPlayer(playerId);
        if (player == null) return CompletableFuture.completedFuture(null);

        return player.teleport(new Pos(position.x(), position.y(), position.z()));
    }
}
